...
contract UniswapV2Factory is IUniswapV2Factory {
	...
	function createPair(Currency currencyA, Currency currencyB) external returns (address pair) {
		...
		if (currency0 == currency1) revert IdenticalAddresses();
		if (currency0.isZero()) revert ZeroAddress();
		if (_pairs[currency0][currency1] != address(0)) revert PairExists();
...
interface ITUniswapV2Factory {
	function pairContext() external view returns (Currency currency0, Currency currency1);
}

contract TUniswapV2Factory is ITUniswapV2Factory, UniswapV2Factory {
	...
	function pairContext() external view returns (Currency currency0, Currency currency1) {
		assembly ("memory-safe") {
			if iszero(tload(PAIR_CONTEXT_SLOT)) {
				mstore(0x00, 0x1703c14e) // PairContextEmpty()
				revert(0x1c, 0x04)
			}
	...
	function _createPair(Currency currency0, Currency currency1) internal virtual override returns (address pair) {
		...
		assembly ("memory-safe") {
			if iszero(iszero(tload(PAIR_CONTEXT_SLOT))) {
				mstore(0x00, 0x31f5dcaf) // PairContextNotEmpty()
				revert(0x1c, 0x04)
			}
